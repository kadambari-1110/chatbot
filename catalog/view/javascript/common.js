...
function renderChatLog() {
    ...
    log.innerHTML = chatMessages.map(msg =>
        `<div class="chat-bubble ${msg.role}">${msg.role === 'user' ? '<strong>You:</strong> ' : '<strong>Bot:</strong> '}${msg.text}</div>`
    ).join("");
...
async function sendChat() {
    ...
    try {
        const res = await fetch("http://192.168.0.109:8000/chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message })
        });
        const data = await res.json();
        chatMessages.push({ role: 'bot', text: data.response });
        renderChatLog();
    } catch (e) {
        chatMessages.push({ role: 'bot', text: '<span style=\'color:red;\'><strong>Bot offline.</strong></span>' });
        renderChatLog();
    }
